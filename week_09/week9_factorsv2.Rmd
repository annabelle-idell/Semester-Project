...
```{r exercise1}
newdata <- gss_cat %>% 
  mutate(income1 = fct_other(rincome, keep = c("Don't know", "No answer", "Refused", "Not applicable")))
```
...
tv <- gss_cat %>% 
  mutate(big_relig = fct_lump(relig, 4))
```
...
tv %>% group_by(year, big_relig) %>% 
  summarise(tv = mean(tvhours, na.rm = T)) %>% 
  ggplot(aes(x = year, y = tv, color = fct_reorder2(big_relig, year, tv))) +
  geom_line() +
  labs(title = "Mean Hours of TV watched: by year by religion")
```
