...
covid <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv") %>% 
  ...
  rename(confirmed = `3/9/23`) #replace the stars with the most recent date in the dataset (i.e. the last column)
```
...
covid_top<- covid %>% 
  slice_max(confirmed, n = 30) %>% #use ?slice_max() to learn what needs to go here
  select(confirmed, everything())#This line reorders the columns so that `confirmed` is first, followed by everything else
```

Now create a plot with a marker placed in the location of each of the top counties
...
```{r}
leaflet(data=covid_top) %>% addProviderTiles(providers$Stadia.StamenTerrain) %>% 
  addMarkers(lng = ~Long_, lat = ~Lat)
```
...
leaflet(data=covid_top) %>% addTiles() %>% 
  ...
  addMarkers(lng = ~Long_, lat = ~Lat, label = ~paste(Admin2, "=", confirmed, "cases"))
...
First the set-up
```{r}
pal<-colorNumeric(palette = c("yellow", "red"), domain = covid_top$confirmed/10)
